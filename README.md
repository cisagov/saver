...
```console
docker run cisagov/saver:1.5.0-rc.1
```
...
1. Create a `compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      saver:
        image: cisagov/saver:1.5.0-rc.1
        volumes:
          - source: <your_log_dir>
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/saver:1.5.0-rc.1
    ```
...
|-----------|-------------|
|`cisagov/saver:1.5.0-rc.1`| An exact release version. |
