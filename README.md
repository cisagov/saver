...
```console
docker run cisagov/saver:1.5.0-rc.2
```
...
1. Create a `compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      saver:
        image: cisagov/saver:1.5.0-rc.2
        volumes:
          - source: <your_log_dir>
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/saver:1.5.0-rc.2
    ```
...
|-----------|-------------|
|`cisagov/saver:1.5.0-rc.2`| An exact release version. |
