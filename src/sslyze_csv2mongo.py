...
def open_csv_files():
    ...
    clean_federal = []
    unique = set()
    with _open_file(CURRENT_FEDERAL_FILE, newline="") as current_federal:
    ...
    agency_dict = {}
    with _open_file(AGENCIES_FILE, newline="") as agencies:
        for row in csv.reader(agencies):
            agency_dict[row[0]] = row[1]

    # Write out the unique agencies
    with _open_file(UNIQUE_AGENCIES_FILE, "w+", newline="") as unique_agencies:
    ...
    with _open_file(CLEAN_CURRENT_FEDERAL_FILE, "w+", newline="") as clean_output:
...
def store_data(clean_federal, agency_dict, db_config_file):
    """Save the sslyze data to the database and delete data older than one year.
    ...
    domains_processed = 0
    with _open_file(SSLYZE_RESULTS_FILE, newline="") as results_file:
        csv_f = csv.DictReader(results_file)
